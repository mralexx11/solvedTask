...
async function startRecursive(users, sum = 0) {
   ...
   if (users.length) {
    let usersSlice = users.splice(0,10);
    let usersSliceAsync = await Promise.all(usersSlice.map(async user => processUser(user.id)));
    let sumId = usersSliceAsync.reduce((sum, id) => sum + id);
    sumId += sum;
    return startRecursive(users, sumId);
